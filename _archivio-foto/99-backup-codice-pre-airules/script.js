...
(function () {
  ...
  /* L'animazione del marchio nell'apertura si ferma quando esce dallo
     schermo e riprende quando rientra: identica da vedere, ma smette di
     far lavorare la GPU mentre si legge il menù mille pixel più in giù. */
  var marchioHero = document.querySelector('.hero__marchio');
  if (marchioHero && 'IntersectionObserver' in window) {
    /* Non si parte in pausa: se per qualsiasi motivo l'osservatore non
       partisse, l'animazione resterebbe ferma per sempre. Cosi' invece il
       caso peggiore e' il comportamento di prima — l'animazione va,
       semplicemente non si mette mai in pausa. */
    new IntersectionObserver(function (voci) {
      voci.forEach(function (v) {
        marchioHero.classList.toggle('hero__marchio--fermo', !v.isIntersecting);
      });
    }).observe(marchioHero);
  }
